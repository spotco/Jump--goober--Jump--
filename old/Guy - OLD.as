		...
		public function update(walls:Array) { //checks wall collisions and updates position based on velocity, the most wizardry here
			...
			for(var i = 0; i < walls.length; i++) { //try moving vy in y direction
				if (hitTestObject(walls[i].hitbox)) {
					...
					vy = -vy/1.1;
					var counter = 0;
					while(hitTestObject(walls[i].hitbox)) { //if hits a wall, move backward vy up to 25 times
						trace("entery");
						y += vy/Math.abs(vy);
						trace(counter+", VY : "+vy);
						counter++;
						if (counter == 25) { //if 25 times and still hitting, reverse direction (maybe going too fast and lodged into wall?)
							vy = -vy;
						}
						if (counter > 200) { //give up
							//y -= 5;
							break;
						}
					}
			...
			for(i = 0; i < walls.length; i++) { //try moving vx, same process as with y
				if (hitTestObject(walls[i].hitbox)) {
					...
					vx = -vx/1.1;
					var counter = 0;
					while(hitTestObject(walls[i].hitbox)) {
						counter++;
						trace("enterx");
						x += vx;
						if (counter == 25) {
